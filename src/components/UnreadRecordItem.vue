<template>
  <div class="unread-recorditem">
   <flexbox :gutter="0" class="head">
       ...
       <flexbox-item :span="3/6" style="color:#ADADAD;text-align:right;display:flex;justify-content:flex-end;">
           <div class="fs-12 text-c" style="border-radius:10px;background:#E0E0E0;width:60px;" :style="{background:isRead?'#01B468':'#E0E0E0', color:isRead?'#fff':''}" @click="setRead(item.ids)">{{`${isRead?'已读':'标记已读'}`}}</div>
       </flexbox-item>
   ...
   <flexbox :gutter="0">
       <flexbox-item :span="3/4" class="fs-12">
           ...
           <span style="color:#ADADAD;">{{ item.custname}}</span>
...
import { URL_DELETE_CUSTOMER_FOLLOW_RECORD, URL_SAVE_CUSTFRDISCUSS, URL_DELETE_CUSTFRDISCUSS, URL_SET_RECORD_READ } from '../constant/url.js';
import { Toast, MessageBox } from 'mint-ui';

export default {
    name: 'unreadRecordItem',
    directives: {
        TransferDom
    },
    components: {
    ...
    methods: {
        ...
        setRead (custFrIds) {
            http.post(URL_SET_RECORD_READ, {
                body: `custFrIds=${custFrIds}`
            })
            .then((res) => {
                console.log(res);
                this.isRead = true;
